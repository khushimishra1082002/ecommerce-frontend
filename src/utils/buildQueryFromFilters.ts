...
//     // If value is array → append multiple times
...
//       });
//     } 
//     // If value is object → flatten (for min/max price, date range etc.)
...
export const buildQueryFromFilters = (filters) => {
  ...
  Object.entries(filters).forEach(([key, value]) => {
    ...
    // 🔥 Fix naming for backend
    if (key === "subcategories") {
      value.forEach((v) => params.append("subcategory", v));
      return;
    }
