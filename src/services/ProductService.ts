...
export const getRecommendedProductData = async (productId) => {
  // try {
  //   const response = await api.get(`${conf.GetRecommendedProduct}/${productId}`);
  //   console.log("API Response:", response.data);
  //   return response.data;
  // } catch (error) {
  //   console.error(
...
export const editProductData = async (productId, formData) => {
  try {
    const response = await api.put(`${conf.editProductUrl}/${productId}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
